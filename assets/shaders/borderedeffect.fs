...
extern MY_HIGHP_OR_MEDIUMP vec3 borderedeffect;
...
vec4 layerTexel( Image texture, vec2 uv, int frame, vec2 displace, float parallax, bool tile ) {
    ...
    vec4 col = vec4(0.0);
    if (tile) {
        duv = mod(duv + vec2(borderedeffect.z * 10.0) / image_details, vec2(69.0 / image_details.x, 69.0 / image_details.y));
...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
    ...
    vec4 maskTex = layerTexel(texture, texture_coords, 1, borderedeffect.xy, 0.0, false);
    ...
    for (float i = 0; i <= 1; i += steps) {
        vec4 layerTex = layerTexel(texture, texture_coords, 4, borderedeffect.xy, 14.0 + mult - i * mult, true);
    ...
    for (float i = 0; i <= 1; i += steps) {
        vec4 layerTex = layerTexel(texture, texture_coords, 3, borderedeffect.xy, 5.0 + mult - i * mult, false);
        layerTex.a *= maskTex.a;
        layerTex.a *= i;
        layerTex.rgb = mix(layerTex.rgb, pink, i * i);
        blendedTex = alphaBlend(layerTex, blendedTex);
    ...
    for (float i = 0; i <= 1; i += steps) {
        vec4 layerTex = layerTexel(texture, texture_coords, 2, borderedeffect.xy, 2.5 + mult - i * mult, false);
        layerTex.a *= maskTex.a;
        layerTex.a *= i;
        layerTex.rgb = mix(layerTex.rgb, pink, i * i);
        blendedTex = alphaBlend(layerTex, blendedTex);
